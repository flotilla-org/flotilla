...
async fn fatal_runtime_observation_fails_a_running_terminal_with_the_credential_slot() {
    ...
    let deps = reconciler.fetch_dependencies(&session).await.expect("observe auth failure");
    let patch = reconciler.reconcile(&session, &deps, Utc::now()).patch.expect("failure patch");
    patch.apply(&mut status);
