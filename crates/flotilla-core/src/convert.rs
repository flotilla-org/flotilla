...
mod tests {
    ...
    use flotilla_protocol::{test_support::qp, HostName, HostProviderStatus, QualifiedPath, WorkItemIdentity, WorkItemKind};
    ...
    fn convert_correlated_checkout() {
        let item = CorrelationResult::Correlated(CorrelatedWorkItem {
            anchor: CorrelatedAnchor::Checkout(CheckoutRef { key: qp("/repos/my-project/wt-1"), is_main_checkout: false }),
            checkout_ref: Some(CheckoutRef { key: qp("/repos/my-project/wt-1"), is_main_checkout: false }),
        ...
        assert_eq!(checkout.key, qp("/repos/my-project/wt-1"));
    ...
    fn convert_correlated_checkout_has_hostname_source() {
        ...
        let item = CorrelationResult::Correlated(CorrelatedWorkItem {
            anchor: CorrelatedAnchor::Checkout(CheckoutRef { key: qp("/repos/proj/wt"), is_main_checkout: false }),
            checkout_ref: Some(CheckoutRef { key: qp("/repos/proj/wt"), is_main_checkout: false }),
            attachable_set_id: None,
    ...
    fn convert_checkout_host_from_host_path() {
        ...
        let item = CorrelationResult::Correlated(CorrelatedWorkItem {
            anchor: CorrelatedAnchor::Checkout(CheckoutRef {
                key: QualifiedPath::from_host_path(&remote_host, PathBuf::from("/repos/proj")),
        ...
        // Should use the checkout's QualifiedPath host, not the local fallback
