...
impl super::CheckoutManager for CloneCheckoutManager {
    async fn list_checkouts(&self, _repo_root: &ExecutionEnvironmentPath) -> Result<Vec<(ExecutionEnvironmentPath, Checkout)>, String> {
        ...
        for entry in output.lines() {
            ...
            let host_path =
                flotilla_protocol::QualifiedPath::from_host_path(&flotilla_protocol::HostName::local(), std::path::Path::new(&dir));
    ...
    async fn create_checkout(
        ...
        let host_path =
            flotilla_protocol::QualifiedPath::from_host_path(&flotilla_protocol::HostName::local(), std::path::Path::new(&checkout_dir));
