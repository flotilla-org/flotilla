...
impl CmuxWorkspaceManager {
    ...
    fn parse_ok_ref(output: &str) -> String {
        output.strip_prefix("OK ").unwrap_or(output).split_whitespace().next().unwrap_or("").to_string()
    }
    ...
    fn parse_window_refs(output: &str) -> Result<Vec<String>, String> {
        ...
        let windows = parsed.as_array().ok_or("cmux list-windows: expected JSON array")?;
...
impl super::WorkspaceManager for CmuxWorkspaceManager {
    ...
    async fn create_workspace(&self, config: &WorkspaceConfig) -> Result<(String, Workspace), String> {
        ...
        let panels_json = self.cmux_cmd(&["--json", "list-panels", "--workspace", &ws_ref]).await?;
        let panels: serde_json::Value = serde_json::from_str(&panels_json).map_err(|e| e.to_string())?;
        let first = panels["surfaces"].as_array().and_then(|s| s.first()).ok_or("cmux list-panels: no surfaces in new workspace")?;
        let initial_surface = first["ref"].as_str().ok_or("cmux list-panels: initial surface missing 'ref'")?.to_string();
        let initial_pane = first["pane_ref"].as_str().ok_or("cmux list-panels: initial surface missing 'pane_ref'")?.to_string();
        ...
        for (i, pane) in rendered.panes.iter().enumerate() {
            let (split_surface_ref, pane_ref) = if i == 0 {
                ...
                let pane_ref = panels["surfaces"]
                    .as_array()
                    .and_then(|surfs| surfs.iter().find(|s| s["ref"].as_str() == Some(&new_surface)))
            ...
            for (j, surface) in pane.surfaces.iter().enumerate() {
                let surface_ref = if j == 0 {
                    ...
                    let output = self.cmux_cmd(&["new-surface", "--type", "terminal", "--pane", &pane_ref, "--workspace", &ws_ref]).await?;
        ...
        for (surface_ref, cmd) in &surface_cmds {
            self.cmux_cmd(&["send", "--workspace", &ws_ref, "--surface", surface_ref, &format!("{cmd}\n")]).await?;
        }

        // Select active surfaces within their panes, then restore tab order
        for (surface_ref, pane_ref, tab_index) in &active_surfaces {
            self.cmux_cmd(&["move-surface", "--surface", surface_ref, "--pane", pane_ref, "--focus", "true", "--workspace", &ws_ref])
                .await?;
            self.cmux_cmd(&["reorder-surface", "--surface", surface_ref, "--index", &tab_index.to_string(), "--workspace", &ws_ref])
                .await?;
        }
