...
use flotilla_resources::{
    ...
    AGENT_ADAPTERS_CAPABILITY, AUTHORITY_LABEL, CONVOY_LABEL, ROLE_LABEL, VESSEL_LABEL, VESSEL_REF_LABEL,
};
        ...
        .create(
            &InputMeta::builder()
                ...
                .labels(BTreeMap::from([
                    ...
                    (CONVOY_LABEL.to_string(), "quartermaster".to_string()),
                ]))
                .build(),
            &flotilla_resources::PresentationSpec {
                convoy_ref: "quartermaster".to_string(),
                presentation_policy_ref: "default".to_string(),
                name: "quartermaster".to_string(),
                process_selector: BTreeMap::from([(CONVOY_LABEL.to_string(), "quartermaster".to_string())]),
            },
        )
        .await
        .expect("presentation");

    daemon.reap_convoy_internal("flotilla", "quartermaster", true).await.expect("convoy teardown");

    assert!(matches!(presentations.get("quartermaster-work").await, Err(ResourceError::NotFound { .. })));
    assert!(matches!(backend.using::<ResourceConvoy>("flotilla").get("quartermaster").await, Err(ResourceError::NotFound { .. })));
}
