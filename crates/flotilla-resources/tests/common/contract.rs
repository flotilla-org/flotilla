...
pub async fn assert_project_definition_optional_field_can_be_cleared_with_backend(backend: ResourceBackend) {
    ...
    original.issue_source_bindings = vec![IssueSource { service: "github".to_string(), scope: "acme/widgets".to_string() }.into()];
    ...
    cleared.issue_source_bindings = Vec::new();
    feta.definitions::<Project>("flotilla").apply(&meta, &cleared).await.expect("clear replicated issue source");
    assert_eq!(
        feta.definitions::<Project>("flotilla").get("widgets").await.expect("get locally cleared Project").spec.issue_source_bindings,
        Vec::new(),
    ...
    for merged in [
        ...
        assert_eq!(merged.spec.issue_source_bindings, Vec::new());
        assert!(merged.metadata.merge.as_ref().expect("definition merge metadata").conflicts.is_empty());
    }
}
