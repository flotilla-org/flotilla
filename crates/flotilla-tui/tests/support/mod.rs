...
pub fn make_checkout(branch: &str, path: &str, is_main: bool) -> (flotilla_protocol::qualified_path::QualifiedPath, Checkout) {
    let key = flotilla_protocol::qualified_path::QualifiedPath::from_host_name(&flotilla_protocol::HostName::local(), PathBuf::from(path));
