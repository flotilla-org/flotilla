...
Each `StepAction` variant maps to a `resolve()` match arm that contains the logic currently in `execute()` or in the closure bodies. The resolver creates helper structs (e.g., `TeleportFlow`, `CheckoutFlow`, `WorkspaceOrchestrator`) from its own fields and the action's data.
