...
- **Dispatchable tickets** (`wayfinder:research`, AFK `wayfinder:task`, and the artifact-building half of `wayfinder:prototype`): dispatch a convoy against the real ticket — `flotilla convoy start --project flotilla --issue <n> --placement-policy <policy> --no-attach`. The convoy is the claim; also set the assignee at dispatch so cross-session frontier queries see it. Artifacts land as PRs where sensible (e.g. prototypes under `prototypes/<map>-<ticket>/`); the standard crew workflow (PR + shepherd, never merge) applies unchanged.
