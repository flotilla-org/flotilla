...
- **Dispatchable tickets** (`wayfinder:research`, AFK `wayfinder:task`, and the artifact-building half of `wayfinder:prototype`): dispatch a convoy against the real ticket — `flotilla convoy start --project flotilla --issue <n> --placement-policy <policy> --no-attach`. The convoy is the claim; also set the assignee at dispatch so cross-session frontier queries see it. Add `--agent [capability=]adapter[:model]` (repeatable; bare form binds the `code` capability) when the ticket should run on a specific harness — `--agent claude-code:opus` for a research ticket that wants the bigger model, `--agent claude-code:sonnet` or `--agent review=codex` when a subscription's budget is spent; see [configuration](../configuration.md#dispatch-time-agent-selection). Artifacts land as PRs where sensible (e.g. prototypes under `prototypes/<map>-<ticket>/`); the standard crew workflow (PR + shepherd, never merge) applies unchanged.
  Prototype tickets are HITL at resolution: the convoy builds the artifact, but the ticket only resolves through human reaction to it — the build is dispatchable, the decision is not.
