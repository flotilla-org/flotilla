...
impl App {
    ...
    pub fn selected_work_item(&self) -> Option<&WorkItem> {
        ...
        match self.model.active().data.table_entries.get(table_idx)? {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                if mouse.modifiers.contains(KeyModifiers::SHIFT) {
                    if let Some(si) = self.row_at_mouse(mouse.column, mouse.row) {
                        let table_idx = self.model.active().data.selectable_indices[si];
    ...
    fn action_enter_multi_select(&mut self) {
        ...
        for &si in &multi_selected {
            if let Some(&table_idx) = self.model.active().data.selectable_indices.get(si) {
                if let Some(TableEntry::Item(item)) = self.model.active().data.table_entries.get(table_idx) {
    ...
    fn select_next(&mut self) {
        let indices = &self.model.active().data.selectable_indices;
        if indices.is_empty() {
            return;
        }
        ...
        let table_idx = self.model.active().data.selectable_indices[next];
    ...
    fn select_prev(&mut self) {
        ...
        let table_idx = self.model.active().data.selectable_indices[prev];
