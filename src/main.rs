...
enum SubCommand {
    ...
    /// Run the standing host-local CodexBar usage observer
    #[command(hide = true)]
    UsageObserver,
...
async fn main() -> Result<()> {
    ...
    let result = match command {
        ...
        Some(SubCommand::UsageObserver) => run_usage_observer(&cli).await,
...
async fn run_usage_observer(cli: &Cli) -> Result<()> {
    let poller = flotilla_core::usage_observer::CodexBarUsagePoller::new(Arc::new(ProcessCommandRunner));
    loop {
        let daemon = connect_daemon(cli).await?;
        for provider in flotilla_core::usage_observer::PROVIDERS {
            let observations = match poller.poll(provider).await {
                Ok(observations) => observations,
                Err(error) => {
                    eprintln!("usage observer: {error}");
                    continue;
                }
            };
            for observation in observations {
                let status = serde_json::to_value(&observation.status)
                    .map_err(|error| color_eyre::eyre::eyre!("encode {provider} usage observation: {error}"))?;
                ...
                .await;
                match result {
                    Ok(CommandValue::Error { message }) => {
                        eprintln!("usage observer: could not publish {provider} account {}: {message}", observation.account);
                    }
                    Ok(_) => {}
                    Err(error) => {
                        eprintln!("usage observer: daemon rejected {provider} account {}: {error}", observation.account);
                    }
                }
            }
        }
        tokio::time::sleep(flotilla_core::usage_observer::POLL_INTERVAL).await;
    }
}
